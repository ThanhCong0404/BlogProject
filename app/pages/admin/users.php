...
function display_form($action, $row = [], $errors = [])
{
?>
        ...
        <form method="post" enctype="multipart/form-data">
            <?php if ($action === 'edit' || $action === 'add'): ?>
                <h1 class="h3 mb-3 fw-normal">
                    <?= $action === 'add' ? 'Create account' : 'Edit account' ?>
                </h1>
            <?php endif; ?>
...
function display_users_table($rows, $PAGE)
{
?>
    <h4>
        Users
...
// Display the appropriate form based on the action
switch ($action) {
    case 'add':
    case 'edit':
    case 'delete':
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $errors = validate_user_input($_POST);
        ...
        display_form($action, $row, $errors);
        break;

    case 'list':
    default:
        $rows = get_users_list();
        $PAGE = get_pagination_data();

        if (!empty($rows)) {
            display_users_table($rows, $PAGE);
        } else {
...
<div class="my-2">
    <label class="d-block">
        <img class="mx-auto d-block image-preview-edit" src="<?= get_image($image_path) ?>" style="cursor: pointer;width: 150px;height: 150px;object-fit: cover;">
